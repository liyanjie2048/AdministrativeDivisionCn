    ...
    class Collector
    {
        ...
        async Task AddCountryAsync()
        {
            ...
            await context.DataSet.InsertOneAsync(new AdministrativeDivisionCn
            {
                ...
                Level = 0,
                Display = "中华人民共和国",
                ChildrenUrl = "http://www.stats.gov.cn/tjsj/tjbz/tjyqhdmhcxhfdm/2020/index.html",
            });
        }
        ...
        static async Task<(bool Success, IEnumerable<AdministrativeDivisionCn> Children)> TryGetChildrenAsync(AdministrativeDivisionCn parent)
        {
            ...
            {
                0 => html => html.DocumentNode.Descendants("tr").Where(_ => _.HasClass("provincetr"))
                ...
                1 => html => html.DocumentNode.Descendants("tr").Where(_ => _.HasClass("citytr"))
                    .Select(_ =>
                    {
                ...
                2 => html => html.DocumentNode.Descendants("tr").Where(_ => _.HasClass("countytr"))
                    .Select(_ =>
                    {
                        var href = _.Descendants("a").FirstOrDefault()?.GetAttributeValue("href", string.Empty);
                        var code = _.Descendants("td").FirstOrDefault().InnerText;
                ...
                3 => html => html.DocumentNode.Descendants("tr").Where(_ => _.HasClass("towntr"))
                    .Select(_ =>
                    {
                        var href = _.Descendants("a").FirstOrDefault()?.GetAttributeValue("href", string.Empty);
                        var code = _.Descendants("td").FirstOrDefault().InnerText;
                ...
                4 => html => html.DocumentNode.Descendants("tr").Where(_ => _.HasClass("villagetr"))
                    .Select(_ =>
                    {
                        var code = _.Descendants("td").FirstOrDefault().InnerText;
                        var display = _.Descendants("td").LastOrDefault().InnerText;
        ...
        static async Task<(bool Success, HtmlDocument HtmlDoc)> TryLoadHtmlAsync(string url)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    htmlDoc.Load(await response.Content.ReadAsStreamAsync(), Encoding.GetEncoding("GB2312"));
                    return (true, htmlDoc);
                }
            }
            catch (Exception e)
